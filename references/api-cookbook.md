...
# 批量查询（POST）— ⚠️ 见下方"实操注意"
curl -s -X POST "https://api.semanticscholar.org/graph/v1/paper/batch?fields=title,year,citationCount,venue" \
  -H "Content-Type: application/json" \
  -d '{"ids":["DOI:10.xxx/xxx","ARXIV:2301.00001"]}' \
  -H "x-api-key: YOUR_KEY"
# ⚠️ 实操注意：无 API Key 时大 batch 会导致 venue/citationCount 字段全部返回 null。
#    建议无 Key 时每批 ≤5 篇，有 Key 可放宽至 20-50 篇。
```
...
**鉴权**：无需（有 API Key 可提升速率，免费申请 https://www.ncbi.nlm.nih.gov/account/settings/）  
**格式**：JSON（esearch/esummary）/ XML（efetch）  
**速率**：无 Key 3 req/s；有 Key 10 req/s；请求加 `&email=your@email.com`

### 两遍策略（推荐）

**第一遍 — 轻量扫描 (esearch + esummary)**：

```bash
# Step 1：esearch — 搜索，获取 PMID 列表（JSON）
curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=tissue+engineering[MeSH]+AND+bioprinting[Title/Abstract]&retmax=20&retmode=json&email=your@email.com"

# Step 2：esummary — 按 PMID 批量获取 JSON DocSum 元数据（轻量，无摘要）
...
```bash
# Step 3：efetch — 批量获取 XML 详情（含完整摘要、MeSH Terms、全量作者）
curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=36517592,37112345&rettype=abstract&retmode=xml&email=your@email.com"

# Step 4（可选）：elink — 获取相关文献、引用关系
...
| 参数 | 说明 | 示例 |
|------|------|------|
| `term` | PubMed 查询表达式 | `CRISPR[MeSH] AND 2024[dp]` |
| `retmax` | 返回条数（上限 10000） | `retmax=20` |
| `retstart` | 分页偏移 | `retstart=20` |
| `sort` | 排序（relevance/date/pub_date） | `sort=date` |
| `datetype` | 日期类型（pdat/mdat/edat） | `datetype=pdat` |
| `mindate/maxdate` | 日期范围（YYYY/MM/DD） | `mindate=2023&maxdate=2024` |
...
| `result[pmid].authors[].name` | authors[]（LastName Initials 格式） |
| `result[pmid].pubdate`（前 4 位） | year |
| `result[pmid].source` | venue（期刊缩写） |
| `result[pmid].fulljournalname` | venue（期刊全称） |
| `result[pmid].articleids[type=doi].value` | doi |
| `result[pmid].articleids[type=pmc].value` | pmcid（PMC 全文 ID，如有） |
| `result[pmid].uid` | pubmed_id |
| `result[pmid].elocationid` | 电子定位 ID（online-first 文章） |
| `result[pmid].pubtype[]` | publication_type |
...
# 标题/摘要自由文本
"organoid"[Title/Abstract] OR "organ-on-a-chip"[Title/Abstract]
...
# 布尔组合
"Tissue Engineering"[MeSH] AND ("bioprinting"[Title/Abstract] OR "scaffold"[Title/Abstract]) AND 2023:2024[dp]
```

### Hermes pubmed skill 快捷方式

```bash
# Hermes 环境推荐直接调用 pubmed skill（含缓存、重试、MeSH 自动扩展）
python3 scripts/pubmed.py --query '"Tissue Engineering"[MeSH]' --max-results 10 --sort date
python3 scripts/pubmed.py --pmids "36517592,37112345"  # 批量 PMID 查详情
```
