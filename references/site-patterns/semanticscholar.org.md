...
- **批量查询时字段丢失**：无 API Key 时，单次 batch 请求中的论文数量过大会导致 `venue` 和 `citationCount` 等字段在全部结果中返回 null / 空白，而非部分丢失。实测 20 篇的 batch 全部 null，拆分为 4-5 篇/batch 后正常返回。建议无 Key 时每批 ≤5 篇，有 Key 时可放宽至 20-50 篇（发现于 2026-06-12，皮肤类器官检索任务）
